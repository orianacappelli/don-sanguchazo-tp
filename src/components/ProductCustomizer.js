...
import AddToCartModal from './AddToCartModal'; // Importamos el modal
...
export default function ProductCustomizer({ product }) {
    ...
    if (product?.customizations?.length > 0) {
      product.customizations.forEach((custom) => {
        ...
        if (isMultiSelect) {
          ...
          initialChoices[custom.name] = custom.defaultOptions?.length > 0 ? custom.defaultOptions[0] : (custom.options?.length > 0 ? custom.options[0] : '');
        }
      });
    }
  ...
  const handleAddToCart = () => {
    ...
    // ABRIMOS EL MODAL EN LUGAR DEL ALERT
    setIsModalOpen(true);
  };
      ...
      <div className="mt-auto border-t border-gray-100 pt-6">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex items-center bg-gray-100 rounded-xl p-1">
            <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="w-10 h-10 rounded-lg bg-white flex items-center justify-center font-black text-gray-600 shadow-sm hover:bg-gray-50">-</button>
            <span className="font-black w-10 text-center text-gray-800">{quantity}</span>
            <button onClick={() => setQuantity(q => q + 1)} className="w-10 h-10 rounded-lg bg-white flex items-center justify-center font-black text-gray-600 shadow-sm hover:bg-gray-50">+</button>
          </div>
        </div>

        <div className="flex gap-4">
          <button onClick={handleAddToCart} className="flex-grow bg-[#157a2c] text-white py-4 rounded-xl font-bold text-xl hover:bg-green-700 transition-colors shadow-lg shadow-green-200">
            Agregar al carrito
          </button>
